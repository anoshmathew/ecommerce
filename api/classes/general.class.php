...
class general
	...
	# Remove Stripslashes Array
	    ...
	    function upload_resize($filed_name,$pref='swebin',$file_type='image',$rez=array(),$unlink="null",$file_size="",$l=array(600,400,'crop',100),$m=array(200,130,'crop',100),$s=array(100,75,'auto',100),$folder="photos")
		{
		     // Example
			// $upload = $objgen->upload_file("banner_name","banner_2");
			 //if($upload[1]!="")
    ...
    Resize, then crop. This is my favourite. Exact size, no distortion. (crop)*/
				...
				if($error=="")
				{
					...
					if($unlink!="null")
					{
					      ...
					      if(file_exists(ROOT_SITE."/".$folder."/".$unlink))
	 						unlink(ROOT_SITE."/".$folder."/orginal/".$unlink);
							
						 if(file_exists(ROOT_SITE."/".$folder."/large/".$unlink))
	 						unlink(ROOT_SITE."/".$folder."/large/".$unlink);
							
							
						 if(file_exists(ROOT_SITE."/".$folder."/medium/".$unlink))
	 						unlink(ROOT_SITE."/".$folder."/medium/".$unlink);
							
							
						 if(file_exists(ROOT_SITE."/".$folder."/small/".$unlink))
	 						unlink(ROOT_SITE."/".$folder."/small/".$unlink);
					...
					move_uploaded_file($_FILES[$filed_name]["tmp_name"],ROOT_SITE."/".$folder."/orginal/".$file_name);
					
				
					 if(count($rez)>0)
					 {
					 
					     $testimage = $this->openImagegen(ROOT_SITE."/".$folder."/orginal/".$file_name);
					       ...
					       $width_chk  = imagesx($testimage);
						    ...
						    $image 		= new resize(ROOT_SITE."/".$folder."/orginal/".$file_name);
					   ...
					   if(in_array('l',$rez))
					   {
						   ...
						   $image->saveImage(ROOT_SITE."/".$folder."/large/".$file_name,$l[3]);

						   }
						   else
						   {
						     copy(ROOT_SITE."/".$folder."/orginal/".$file_name,ROOT_SITE."/".$folder."/large/".$file_name);
						   }
					   }
					   if(in_array('m',$rez))
					   {
					    ...
					    $testimage = $this->openImagegen(ROOT_SITE."/".$folder."/orginal/".$file_name);
						 ...
						 //  $image 		= new resize(ROOT_SITE."/".$folder."/".$file_name);
						   ...
						   $image->saveImage(ROOT_SITE."/".$folder."/medium/".$file_name,$l[3]);
						   
						    }
						   else
						   {
						     copy(ROOT_SITE."/".$folder."/orginal/".$file_name,ROOT_SITE."/".$folder."/medium/".$file_name);
						   }
					   }
					   if(in_array('s',$rez))
					   {
						  ...
						  // $image 		= new resize(ROOT_SITE."/".$folder."/".$file_name);
						   ...
						   $image->saveImage(ROOT_SITE."/".$folder."/small/".$file_name,$l[3]);
						   
						    }
						   else
						   {
						     copy(ROOT_SITE."/".$folder."/orginal/".$file_name,ROOT_SITE."/".$folder."/small/".$file_name);
						   }
					   }
		...
		function dashboard_count($table,$title,$color,$image,$file,$where="")
		{
          ...
          <div class="small-box bg-'.$color.'">
            ...
            <a href="'.URLAD.$file.'" class="small-box-footer">More Info <i class="fa fa-arrow-circle-right"></i></a>
          </div>
        </div>';
