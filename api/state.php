...
if($authkey==true)
{
	...
	if($attr_id!="")
	{
		$where .=  " AND country_id=".$cat_id;
	}
	
	$attr_details   	= $objgen->get_AllRowscnt("states",$where);
	if($attr_details>0)
	{
		$attr_arr 	= $objgen->get_AllRows("states",0,$attr_details,"attr_id asc, id asc",$where);
		$data		= array();
		...
		{
			$key = $key+1;
			$result     					= $objgen->get_Onerow("countries"," and id=".$val['attr_id']);
