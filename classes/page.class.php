...
//                    else
    ...
    function setPageNo()
    {
        ...
        if ($this->dispType == 'PG_NUMBERED') {
		 ...
		 elseif ($this->dispType == 'PG_BOOSTRAP_AD') {
             
			$this->dispString = $this->dispString."<ul class='pagination pagination-sm m-0 float-right'>";
            If ($prev >= 1)
                $this->dispString = $this->dispString.
       				"<li $this->pclass ><a href = '$this->pageLink&$this->pageVar=$start' class='page-link'> &laquo;</a> <a href = '$this->pageLink&$this->pageVar=$prev' class='page-link'> Prev </a></li>";
            Else
                $this->dispString = $this->dispString."  ";
            For ($indexNo=$start; $indexNo <= $end; $indexNo++) 
                If ($indexNo <> $this->currPage)
                    $this->dispString = $this->dispString.
                        "<li $this->pclass ><a href = '$this->pageLink&$this->pageVar=$indexNo' class='page-link' >$indexNo</a></li>";
                Else
                    $this->dispString = $this->dispString."<li $this->pclass  ><a href=# class='page-link page-link-active'  style='cursor:text' title='Current Page'>$indexNo</a></li>";
            If ($next > $maxPages)
                $this->dispString = $this->dispString."  ";
            Else
                $this->dispString = $this->dispString.
                    "<li $this->pclass><a href = '$this->pageLink&$this->pageVar=$next' class='page-link'>Next</a> <a href = '$this->pageLink&$this->pageVar=$maxPages' class='page-link' >&raquo;</a></li>";
