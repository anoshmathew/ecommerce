...
{    
	...
	$errors  		= array();
	$rules			= array();
	$rules[] 		= "required,category_id,Enter the category Name";
	$rules[] 		= "required,sub_category,Select the Sub Category";
	$errors  		= $objval->validateFields($_POST, $rules);
	
	$brd_exit   	= $objgen->chk_Ext("sub_category","sub_cat_name='$sub_category' and cat_id='$category_id' and id<>$id");
   ...
   if(empty($errors))
	{
		$msg 		= $objgen->upd_Row('sub_category',"sub_cat_name='".$sub_category."',cat_id='".$category_id."',status='".$status."'","id=".$id); 			 
