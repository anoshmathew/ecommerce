...
if($_GET['msg']==1)
{
  $msg2       = "News Created Successfully.";
...
if(isset($_GET['delimg']))
{
	...
	$msg = $objgen->upd_Row('news',"image=''","id=".$id);
...
if(isset($_GET['delimg2']))
{
     $id      = $_GET['delimg2'];
	 $edit    = $_GET['edit'];
	 $result     		= $objgen->get_Onerow("news_img","AND id=".$id);
	 $photo     		= $objgen->check_tag($result['image']);
    ...
    $msg     = $objgen->del_Row("news_img","id=".$id);
   if($msg=="")
   {
	 header("location:".$add_url."/?msg=3&edit=".$edit."&page=".$page);
   }
...
if(isset($_POST['Create']))
{
   ...
   $created_date            = $objgen->check_input($_POST['created_date']); 
   $date			 = date('y-m-d');
 
   $rules		     = array();
   $rules[] 	     = "required,title,Enter the Title";
   $rules[]          = "required,description,Enter the Description";
   $rules[]          = "required,created_date,Enter the Date";

   $errors  	     = $objval->validateFields($_POST, $rules);
  
    $brd_exit = $objgen->chk_Ext("news","title='$title'");
	if($brd_exit>0)
	{
		$errors[] = "This News is already exists.";
	...
	if(empty($errors))
	{
		...
		{
			$upload = $objgen->upload_resize("main_img","news","image",array('l','m','s'),"null","",array(800,800,'auto',100),array(200,200,'auto',100),array(100,100,'auto',100));
		...
		$msg = $objgen->ins_Row('news','title,description,image,news,created_date',"'".$title."','".$objgen->baseencode($description)."','".$main_img."','".$pages."','". $created_date."'");
			...
			for($i=0; $i < count($_FILES['subfile']['tmp_name']); $i++)
			{
				...
				if($_FILES['subfile']['name'][$i]!="")
				{
					...
					if(copy($_FILES['subfile']['tmp_name'][$i],'../photos/orginal/'.$sub_img))
					{
						...
						$msg = $objgen->ins_Row('news_img','news_id,image',"'".$insrt."','".$sub_img."'");
					}             
				}           
			}
...
if(isset($_GET['edit']))
{
	...
	$created_date       = $objgen->check_tag($result['created_date']);
	$date			 = date('y-m-d');
	$main_img		    = $objgen->check_tag($result['image']);

	
	$where 		= " and news_id=".$id;
	$img_count 	= $objgen->get_AllRowscnt("news_img",$where);
	if($img_count>0)
	{
		$img_arr = $objgen->get_AllRows("news_img",0,$img_count,"id asc",$where);
	}
...
{    
	...
	$brd_exit 		= $objgen->chk_Ext("news","created_date='$created_date' and id <>".$id);
	

	   if($_FILES["main_img"]["name"]!="")
			{
				$upload = $objgen->upload_resize("main_img","news","image",array('l','m','s'),$main_img,"",array(308,441,'auto',100),array(150,150,'auto',100),array(100,100,'auto',100));
		...
		$msg = $objgen->upd_Row('news',"title='".$title."',description='".$objgen->baseencode($description)."',image='".$main_img."',created_date='".$created_date."'","id=".$id);
		...
		{ 
			...
			if(count($_FILES['subfile']['tmp_name'])>0)
			{
				for($i=0; $i < count($_FILES['subfile']['tmp_name']); $i++)
				{
					if($_FILES['subfile']['name'][$i]!="")
					{
						$sub_img = created_date("YmdHis").$_FILES['subfile']['name'][$i];
						...
						if(copy($_FILES['subfile']['tmp_name'][$i],'../photos/orginal/'.$sub_img))
						{
							...
							$msg	= $objgen->ins_Row('news_img','image,news_id',"'".$sub_img."','".$id."'");
						}
					}             
				}           
			}
	...
	<div class="row hide_div" >
		...
		<form role="form" method="post" enctype="multipart/form-data" >
			...
			<div class="row">
				...
				<div class="col-md-12">
					<div class="form-group">
						<label for="date">Created Date <span class="error" style="color:red;">*</span></label>
						<input type="date" class="form-control" name="created_date" id="created_date" value="<?=$created_date?>" required>
					</div>
				<div class="col-md-4">
					<div class="form-group">
